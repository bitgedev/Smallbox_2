...
//		      alert($(".timeButton").val());
	...
	function onClk(seat) {
		lock(seat);
// 		alert("좌석 : " + seat);
		document.seatForm.seat.value=seat;

		var strseat = seat.toString();
		var line = strseat.substr(0, 1); 
		var seatno = strseat.substr(-2, 2); 
...
<!-- 빈 좌석 파일 image폴더에 seat0 이라는 이름으로 덮어쓰기 하면 바로 반영 됩니다!! 지금은 걍 별모양임-->
<h2>reserve</h2>
	<form action="ReserveSelectSeatPro.mv" method="post" name ="seatForm">
		
		<div id="info">
<!-- 		임시로 value값에 블팬 넣음 -->
		<h3>영화 제목 : </h3>
		<input type="hidden" name="theater_title" value="블팬">
		<h3>영화 포스터 영역</h3>
		<h3>예약 날짜 : </h3>
		<input type="hidden" name="theater_date" value="2022-12-29">
		<h3>상영 시간 : </h3>
		<input type="hidden" name="theater_time" value="13:00:00">
		<h3>예약 한 id : ${sId }</h3>
		<input type="hidden" name="theater_idx" value="6">
		</div>
...
<!-- 	인원 선택 부분 -->
<!-- 	성인: <input type="text" name="adult"> -->
<!-- 	청소년: <input type="text" name="teen"> -->
<!-- 	우대: <input type="text" name="elder"> -->
	<br>
	<br>
	...
	<input type="hidden" name="teen" value="0">
